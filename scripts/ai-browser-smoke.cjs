...
async function mock(){await ev(()=>{
  ...
  window.fetch=async function(url,opts){
    ...
    if(b.tools||b.tool_choice||b.parallel_tool_calls!==undefined)throw Error('不应发送工具请求');
    if(!b.stream)return new Response(JSON.stringify({choices:[{message:{content:t.body},finish_reason:'stop'}]}),{headers:{'content-type':'application/json'}});
    const frames=[{choices:[{delta:{content:t.body.slice(0,2)}}]},{choices:[{delta:{content:t.body.slice(2)},finish_reason:'stop'}]}];
...
(async()=>{
 ...
 await ev(()=>{const s=document.querySelector('#app').__vue_app__._instance.proxy.$.setupState;window.__s=s;s.nsfwSettings.enabled=false;
   const r=s.createConnectionProfile({name:'离线模拟',templateId:'openai-compatible',baseUrl:'https://moyun-test.invalid/v1',defaultModel:'gemini-test',apiKey:'test-only',lastTest:{status:'ok'}});if(!r.ok)throw Error(r.reason);s.connectionCenter.defaultProfileId=r.profile.id;s.settings.geminiReplyInTool=true;s.showSettings_modal=true;s.activeSettingsTab='context';s.saveData();});await delay(300);
 for(const width of [1280,390]){
   await send('Emulation.setDeviceMetricsOverride',{width,height:844,deviceScaleFactor:1,mobile:width<500});await delay(250);
   await record(width+' 抗截断已移除/二轮补写保留/无溢出',async()=>{const d=await ev(()=>{const a=document.querySelector('[data-settings-second-round-toggle]');a.scrollIntoView({block:'center'});return{removed:!document.querySelector('[data-settings-gemini-reply-toggle]'),secondRound:!!a,overflow:document.documentElement.scrollWidth>innerWidth};});assert.equal(d.removed,true);assert.equal(d.secondRound,true);assert.equal(d.overflow,false);return d;});
   const shot=await send('Page.captureScreenshot',{format:'png'});fs.writeFileSync(path.join(out,'v15-settings-'+width+'.png'),Buffer.from(shot.data,'base64'));
 }
 await record('旧版开关开启的存档刷新后不恢复入口',async()=>{await delay(2600);await send('Page.reload');await delay(400);await ready();await delay(3000);await dismissGuides();await ev(()=>{window.__s=document.querySelector('#app').__vue_app__._instance.proxy.$.setupState;});assert.equal(await ev(()=>window.__s.settings.geminiReplyInTool),true);assert.equal(await ev(()=>!!document.querySelector('[data-settings-gemini-reply-toggle]')),false);});
 await mock();
 await record('11个文本模块×流式/非流式均用普通请求生成',async()=>{const data=await ev(async()=>{const s=window.__s,rows=[];for(const key of ['writing','supplement','settings','character','imagetext','outline','suggestion','review','summary','comments','translate'])for(const stream of [true,false]){const r=s.getModuleRequestConfig(key);const result=await s.fetchAdapterCompletion(r,[{role:'user',content:'输出测试'}],{stream});const req=window.__moyunTest.requests.at(-1);rows.push({key,stream,text:result.text,sentStream:req.stream,tools:!!req.tools,toolChoice:!!req.tool_choice});}return rows;});assert.equal(data.length,22);data.forEach(d=>{assert.equal(d.text,'正文甲乙');assert.equal(d.sentStream,d.stream);assert.equal(d.tools,false);assert.equal(d.toolChoice,false);});return data;});
 await record('两个模型名切换且旧开关为开启仍无工具请求',async()=>{const rows=await ev(async()=>{const s=window.__s,p=s.connectionCenter.profiles.find(p=>p.id===s.connectionCenter.defaultProfileId),rows=[];for(const model of ['gemini-3.8-flash','[AN]gemini-3.8-flash']){p.defaultModel=model;const result=await s.fetchAdapterCompletion(s.getModuleRequestConfig('writing'),[{role:'user',content:'测试'}],{stream:true});rows.push({model,text:result.text,tools:!!window.__moyunTest.requests.at(-1).tools});}return rows;});rows.forEach(r=>{assert.equal(r.text,'正文甲乙');assert.equal(r.tools,false);});return rows;});
 ...
 await record('大纲独立入口普通输出及过滤不重试',async()=>{
 ...
 assert.equal(errors.length,0);fs.writeFileSync(path.join(out,'v15-browser-results.json'),JSON.stringify({passed:true,results,errors},null,2));console.log('BROWSER PASSED:',results.length);ws.close();
})().catch(e=>{console.error(e);fs.writeFileSync(path.join(out,'v15-browser-results.json'),JSON.stringify({passed:false,results,errors,error:String(e)},null,2));ws?.close();process.exitCode=1;});
