...
const announcementId = 'web-2026-09-23-0923-update';
...
(async()=>{
  ...
  for(const width of [1280,390]) {
    ...
    await waitFor(async()=> (await title())==='0923更新');
    ...
    assert.ok(!/抗截断|工具回复|空回复最多/.test(d.text));assert.ok(d.text.includes('AI 输出格式识别更稳健'));
    assert.ok(!/反标记|NSFW|大书保存|0914更新|v?0\.0\.\d+/.test(d.text));
    ...
    const shot=await send('Page.captureScreenshot',{format:'png'});fs.writeFileSync(path.join(out,'announce-0923-'+label+'-'+width+'.png'),Buffer.from(shot.data,'base64'));
    ...
    await send('Page.reload');await waitFor(async()=> (await title())==='0923更新');
    await delay(4500);await clickAck();
  ...
  fs.writeFileSync(path.join(out,'announce-0923-'+label+'.json'),JSON.stringify({base,passed:true,results,errors},null,2));
  console.log('ANNOUNCEMENT PASSED:',results.length);ws.close();
})().catch(e=>{console.error(e);fs.writeFileSync(path.join(out,'announce-0923-'+label+'.json'),JSON.stringify({base,passed:false,results,errors,error:String(e)},null,2));ws?.close();process.exitCode=1;});
